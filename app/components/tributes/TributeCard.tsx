...
export function TributeCard({ tribute, isOwner, onEdit, onDelete, onTogglePremium }: TributeCardProps) {
  return (
    <Link href={`/homenaje/${tribute.slug || tribute.id}`} className="block group">
      <div className="pointer-events-auto">
        <div className="flex flex-col items-center relative group">
          ...
          <h3 className="font-andika text-xl text-primary text-center mb-1">{tribute.nombre}</h3>
          <p className="font-montserrat text-sm text-primary/80 text-center">
            {formatDate(tribute.fecha_nacimiento)} - {formatDate(tribute.fecha_fallecimiento)}
          </p>
        </div>
      </div>
    </Link>
